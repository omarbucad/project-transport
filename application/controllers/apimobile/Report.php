...
class Report extends CI_Controller {
 ...
 //        return $year."/".$month.'/'.$name;
   ...
   //          $limit = (isset($data->limit)) ? $data->limit : 4;
			...
			$this->db->select("r.report_id , r.report_number , r.vehicle_registration_number , r.start_mileage , r.end_mileage , r.report_notes  , r.created, r.report_by, r.pdf_path, r.pdf_file,   r.isFullTank, r.currentMileage, r.currentMileageImage, r.fuelFilled, r.fuelFilledImage, c.vehicle_type_id ,vt.type, s.store_name,s.logo_image_name, s.logo_image_path");
			$this->db->select("u.display_name , u2.display_name as updated_by");
			$this->db->select("rs.status , rs.notes as status_notes , rs.signature");
			$this->db->select("c.checklist_name");
			$this->db->join("user u" , "u.user_id = r.report_by");
